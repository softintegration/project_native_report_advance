...
class ProjectNativeGanttReport(models.AbstractModel):
    _name = 'report.project_native_report_advance.native_gantt_report'
    ...
    def min_max_range(self, task_ids):
        ...
        f_date_start = "date_planned_start"
        f_date_end = "date_planned_finished"
    ...
    def get_gantt(self, parent, y_size_mm=0, x_size_m=0):
        ...
        f_date_start = "date_planned_start"
        f_date_end = "date_planned_finished"
        f_date_deadline = "date_planned_finished"

        date_start, date_end = self.min_max_range(parent.planned_workorder_ids)
        ...
        domain = [('planning_id', '=', parent.id)]
        arch_workorders = self.env['mrp.workorder'].sudo().search(domain)
        workorders_list = arch_workorders

        workorders_list_len = len(workorders_list)
        workorders_list_int = 0
        for item in workorders_list:
            workorders_list_int = workorders_list_int + 1
            ...
            duration = "-"
            if item.calendar_duration_expected:
                m, s = divmod(item.calendar_duration_expected, 60)
                h, m = divmod(m, 60)

                duration = '{:d}:{:02d}:{:02d}'.format(int(h), int(m), int(s))
            ...
            item_dict["color_gantt_set"] = False #item.color_gantt_set
            item_dict["color_gantt"] = 100 #item.color_gantt

            item_dict["is_milestone"] = False#item.is_milestone

            item_dict["on_gantt"] = True#item.on_gantt

            item_dict["schedule_mode"] = "manual" #item.schedule_mode
            item_dict["constrain_type"] = "asap"#item.constrain_type

            padding_depth = 15
            paddepth = 0
            name_css = "col_gantt_name"
            is_group = False
            if True is False:#item.subtask_count > 0:
                name_css = "col_gantt_name task-gantt-items-group"
                is_group = True

            level = 2#item.sorting_level
            ...
            if el_x_sum < page_y_size and workorders_list_int != workorders_list_len:

                el_on_page.append(item_dict)

            else:

                if workorders_list_int == workorders_list_len:
                    el_on_page.append(item_dict)
    ...
    def render_html(self, docids, data=None):
        ...
        return self.env['report'].render('project_native_report_advance.native_gantt_report', docargs)"""
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            'doc_ids': docids,
            'doc_model':'mrp.production.planning',
            'docs': self.env['mrp.production.planning'].browse(docids),
            'get_gantt': self.get_gantt,
            'data': data,
        }
